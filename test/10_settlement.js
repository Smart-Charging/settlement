...
contract('Settlement', function (accounts) {
  ...
  const cdrId = 'c2d7f3a7-ed43-4780-8e86-ab73654e081a'
  ...
  it('can transfer tokens given a signed transfer message', async () => {
    ...
    let sig = await sign(cdrId, accounts[1], '2e28', token.address, wallet)

    const tx = await settlement.transfer(cdrId, accounts[1], toBN('2e28'), token.address, sig.v, sig.r, sig.s, {from: accounts[2]})
  ...
  it('reverts the transfer on insufficient funds', async () => {
    ...
    let sig = await sign(cdrId, accounts[1], '2e29', token.address, wallet)

    try {
      await settlement.transfer(cdrId, accounts[1], toBN('2e29'), token.address, sig.v, sig.r, sig.s, {from: accounts[2]})
      assert.fail()
    } catch (e) {
  ...
  it('reverts the transfer if previously executed for a given session id', async () => {
    ...
    try {
      await settlement.transfer(cdrId, accounts[1], toBN('2e28'), token.address, sig.v, sig.r, sig.s, {from: accounts[2]})
      await settlement.transfer(cdrId, accounts[1], toBN('2e28'), token.address, sig.v, sig.r, sig.s, {from: accounts[2]})
      assert.fail()
    } catch (e) {
