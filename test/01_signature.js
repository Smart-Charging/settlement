...
contract('CheckSignature', function (accounts) {
  ...
  const cdrId = '349cecda-7788-4d4b-abaf-8419a9c39e23'
  const wallet = new ethers.Wallet(privateKey)

  const txMsg = web3.utils.soliditySha3(cdrId, accounts[1], toBN('2e28'), accounts[2]).substr(2)
  ...
  it('gets the signer out of a signed bytes32 message', async () => {
    assert.equal(txMsg, (await sigCheck.getHash(cdrId, accounts[1], toBN('2e28'), accounts[2])).substr(2))
    let messageHashBytes = ethers.utils.arrayify('0x' + txMsg)
    let flatSig = await wallet.signMessage(messageHashBytes)
    let sig = ethers.utils.splitSignature(flatSig)

    assert.equal(account.toLowerCase(),
      (await sigCheck.getTransferSigner(cdrId, accounts[1], toBN('2e28'), accounts[2], sig.v, sig.r, sig.s)).toLowerCase())
  })

  it('gets the signer from signed hash', async () => {
    let sig = await sign(cdrId, accounts[1], '2e28', accounts[2], wallet)
    assert.equal(account.toLowerCase(),
